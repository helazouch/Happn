...
export async function askGemini(prompt: string, versionId: string): Promise<string> {
  try {
    ...
    const contextPrompt = `
      ...
      Voici les détails spécifiques de la version que l'utilisateur consulte:
      ...
      L'utilisateur pose la question suivante: "${prompt}"
      
      Réponds de manière précise en tenant compte de ces informations contextuelles.
